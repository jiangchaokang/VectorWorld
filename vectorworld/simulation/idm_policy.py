...
from vectorworld.utils.data_helpers import modify_agent_states
...
class IDMPolicy:
    ...
    def _initialize_ego_path(self, actor_id):
        ...
        if self.is_planner:
            # On-line/ off-line scenes, roote_lane_indices may not exist or are None.
            # - If exists and is not None, use directly (offline pre-processed line path)
            # - Otherwise return to search for a reasonable line sequence based on the root point + line_graph
            route_lane_indices = self.scenario_dict.get("route_lane_indices", None)
            if route_lane_indices is not None:
                ego_path = route_lane_indices
            else:
                ...
                # Search all lanes
                start_lane, _, _ = self._get_closest_lane_from_position(
                    ego_route_points[0], self.lane_geometries
                )
                end_lane, _, _ = self._get_closest_lane_from_position(
                    ego_route_points[-1], self.lane_geometries
                )
                ego_path = [start_lane]
                ...
                for point in ego_route_points[1:]:
                    if len(ego_path) == 0:
                        # broken lane graph fallback: revert to just using the start lane
                        ego_path = [start_lane]
                        break
                    ...
                    if end_lane in lane_candidates:
                        ...
                        lane_id, _, dist = self._get_closest_lane_from_position(
                            point, self.lane_geometries[lane_candidates]
                        )
