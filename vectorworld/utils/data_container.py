...
def get_features(data):
    ...
    This function is used by downstream components (LDM, sim env, metrics).
    It always exposes **only the 7D static agent state**:

        [x, y, speed, cosθ, sinθ, length, width]

    even if `data['agent'].x` internally contains extra dimensions (e.g. motion code).
    """
    x_agent_states_full = data['agent'].x.float()
    x_agent_types = data['agent'].type.float()

    # Only keep the first 7 static dimensions for downstream use.
    if x_agent_states_full.shape[1] > 7:
        x_agent_states = x_agent_states_full[:, :7]
    else:
        x_agent_states = x_agent_states_full
...
def get_features_with_motion(data):
    """Extract lane/agent features **including motion code** for AE encoder/decoder.

    Agent feature layout:

        - static state: 7D [x, y, speed, cosθ, sinθ, length, width]
        - movement code: 2K dimensions perlyline [x1, y1,..., xK, yK] (aligned to [x]-1I'm sorry, I'm sorry.
        - type one-hot: 3D

    If `data['agent'].motion` is absent, the function falls back to using
    only the static 7D state (backwards compatible).
    """
    x_agent_states = data['agent'].x.float()
    x_agent_types = data['agent'].type.float()

    if hasattr(data['agent'], "motion"):
        x_motion = data['agent'].motion.float()
        x_agent_states_full = torch.cat([x_agent_states, x_motion], dim=-1)
    else:
        x_agent_states_full = x_agent_states

    x_agent = torch.cat([x_agent_states_full, x_agent_types], dim=-1)

    x_lane_states = data['lane'].x.float()
    if 'type' in data['lane']:
        x_lane_types = data['lane'].type.float()
        x_lane = torch.cat(
            [x_lane_states.reshape(-1, x_lane_states.shape[-2] * x_lane_states.shape[-1]), x_lane_types],
            dim=-1
        )
    else:
        x_lane_types = None
    ...
    assert x_agent_states_full.dtype == torch.float32
    assert x_agent_types.dtype == torch.float32
    assert x_lane_states.dtype == torch.float32
    assert x_lane.dtype == torch.float32
    assert x_lane_conn.dtype == torch.float32

    return x_agent, x_agent_states_full, x_agent_types, x_lane, x_lane_states, x_lane_types, x_lane_conn
