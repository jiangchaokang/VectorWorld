...
import random
from vectorworld.utils.lane_graph_helpers import resample_polyline
from vectorworld.utils.geometry import normalize_lanes_and_agents
...
from vectorworld.utils.metrics_helpers import get_lane_length
from configs.config import LANE_CONNECTION_TYPES_WAYMO, LANE_CONNECTION_TYPES_NUPLAN, PARTITIONED


def normalize_and_crop_scene(cond_d, new_d, normalize_dict, cfg, dataset_name,
                             motion_dim: int = 6,
                             num_upsample_points=1000,
                             min_lane_length=0.1):
    ...
    ----------
    motion_dim : int
        Placeholder motion code dimension (must match ae.model.motion_dim).
    """
    ...
    # Provide zero motion code as placeholder (shape: Na x motion_dim)
    new_d['agent'].motion = from_numpy(np.zeros((num_agents, motion_dim), dtype=np.float32))
