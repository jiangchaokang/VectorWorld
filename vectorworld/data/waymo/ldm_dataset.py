...
from vectorworld.utils.data_helpers import sample_latents, reorder_indices
...
class WaymoDatasetLDM(Dataset):
    def __init__(self, cfg: Any, split_name: str = "train") -> None:
        ...
        filelist_root = getattr(self.cfg, "filelist_dir", None)
        if filelist_root is None:
            filelist_root = os.path.join(self.cfg.dataset_path, "filelists")
        os.makedirs(filelist_root, exist_ok=True)

        filelist_path = os.path.join(filelist_root, f"{self.split_name}.txt")
        ...
        if os.path.isfile(filelist_path):
            ...
            if len(files) == 0:
                raise RuntimeError(
                    f"[WaymoDatasetLDM] Filelist '{filelist_path}' exists but is empty."
                )
        else:
            pattern = os.path.join(self.dataset_dir, "*.pkl")
            files = sorted(glob.glob(pattern))

            if len(files) == 0:
                raise RuntimeError(
                    f"[WaymoDatasetLDM] No .pkl files found in '{self.dataset_dir}' "
                    f"when globbing with pattern '{pattern}'."
                )
            ...
            try:
                with open(filelist_path, "w") as f:
                    for p in files:
                        f.write(p + "\n")
                print(
                    f"[WaymoDatasetLDM] Built filelist for split='{self.split_name}' "
                    f"with {len(files)} files -> {filelist_path}"
                )
            except Exception as e:
                print(
                    f"[WaymoDatasetLDM] Warning: failed to write filelist to "
                    f"'{filelist_path}': {e}"
                )
